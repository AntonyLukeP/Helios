...
It occupies the gap between "roll your own with cron + Postgres" and "adopt Temporal" â€” lightweight, developer-friendly, and with first-class support for human-in-the-loop approval steps and a live operational dashboard.

---

## Current Phase

**Phase 0 â€” Repository Skeleton**
...
â†’ **[Getting started: clone, run tests, apply formatting](docs/getting-started.md)**

---

## Implementation

This project is built **entirely from scratch** â€” no scaffolding tools, no starter templates.
...
The unit tests are the executable demonstration in Phase 1. The full call path in one line:

```java
new HeliosEngine().run(new GreetingWorkflow(), "Ada")  // returns "Hello, Ada!"
```

`HeliosEngine` handles the invocation boundary; `GreetingWorkflow` owns the business logic.
Run `.\gradlew.bat test` to see all 12 tests pass.
...
**CI:** GitHub Actions runs `./gradlew check` on every push and pull request targeting `main`. The workflow is defined in [`.github/workflows/ci.yml`](.github/workflows/ci.yml). It contains no deployment, publishing, secrets, or external service connections â€” only validation.
